<?php
	...
	function rollback() {
		...
		header("Location: jadwal-susulan.php?q=");
		exit();
	}
	...
	} else { header('Location: jadwal-susulan.php'); exit(); }
	...
	if($j_s->isi == 0) { header('Location: jadwal-susulan.php'); exit(); }*/
	...
	if(!empty($_POST['pr']) && !empty($_POST['j'])) {
		...
		if($keanggotaan[$pr] != 1) { header('Location: jadwal-susulan.php'); exit(); }
		...
		//echo $query;
		$_SESSION['o']= 1; mysqli_commit($connect); header('Location: jadwal-susulan.php');
	} else { header('Location: jadwal-susulan.php'); exit(); }
?>
